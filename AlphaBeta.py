...
def Min_Max(Board,depth,turn,alpha,beta):
    global count
    if(Success_Game(Board)):
        return 1
    if(Lose_Game(Board)):
        ...
        for i in range(Side):
            for j in range(Side):
                if (Board[i][j]=='-'):
                    ...
                    count=count+1
                    best = max(best, Min_Max(TempoararyBoard,depth,"1",alpha,beta))
                    #for i in range(Side):
                         #print(Board[i])
                    if turn == '2':
                        if best > alpha:
        ...
        for i in range(Side):
            for j in range(Side):
                if(Board[i][j]=="-"):
                    ...
                    count=count+1
                    best=min(best,Min_Max( TempoararyBoard,depth,"2",alpha,beta))
                    #for i in range(Side):
                            # print(Board[i])
                    if turn == '2':
                        if best > alpha:
    ...
    for i in range(Side):
        for j in range(Side):
            ...
            if (Board[i][j] =='-') :
                ...
                move=(Min_Max(TemporaryBoard,depth,"2",-2,2))
                if(Combest>move):
                    Combest=move
                    Comlevel=depth
    return Comlevel
